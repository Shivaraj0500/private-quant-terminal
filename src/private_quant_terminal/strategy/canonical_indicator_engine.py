...
class CanonicalIndicatorEngine:
    ...
    def calculate(
        ...
        spec = self._registry.resolve_spec(expression.name)

        valid_outputs = {output.name.strip().lower() for output in spec.outputs}
        ...
        provider = self._registry.resolve_provider_for(spec)
    ...
    def _validate_result(
        ...
        if not result.outputs:
            raise ValueError(f"Indicator {spec.id} returned no outputs")

        for output_name, series in result.outputs.items():
            if not isinstance(series, TimeSeries):
                raise TypeError(f"Indicator {spec.id} output {output_name} must be a TimeSeries")

            if len(series) != len(candles):
                raise ValueError(
                    f"Indicator {spec.id} output {output_name} is not aligned with candles"
                )
